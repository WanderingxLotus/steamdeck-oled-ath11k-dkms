...
{
	struct ath11k_base *ab = container_of(t, struct ath11k_base, mon_reap_timer);
	int i;
...
static void ath11k_dp_rx_frag_timer(struct timer_list *t)
{
	/* Kernel compat: replace timer_container_of with container_of */
	struct dp_rx_tid *rx_tid = container_of(t, struct dp_rx_tid, frag_timer);
