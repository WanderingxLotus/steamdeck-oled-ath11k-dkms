...
{
	/* Kernel < 6.16: no timer_container_of(); use container_of() */
	struct ath11k_hp_update_timer *hp_upd =
	container_of(t, struct ath11k_hp_update_timer, timer);
	struct ath11k_base *ab = hp_upd->ab;
	struct hal_srng *srng = &ab->hal.srng_list[hp_upd->ring_id];
	 ...
	 */
	if (hp_upd->timer_tx_num != hp_upd->tx_num) {
		hp_upd->timer_tx_num = hp_upd->tx_num;
		mod_timer(&hp_upd->timer, jiffies +
		  msecs_to_jiffies(hp_upd->interval));
	} else {
		hp_upd->started = false;
